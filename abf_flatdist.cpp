/*
This program predicts the flux of Yb or Mg 
that is produced at the viewport over a 
range of oven tempertures in the SADiCS experiment.
The data is written to a file

Kristen approved, latest version, 12/09/2016
*/
...
int main() {
 ...
 /*  
    ...
    //--- fid_vol = fiducial volume, n3 = 3d number density of atoms, num_fid = number of atoms in the fiducial volume

    //    double fid_vol = Pi*w_a*w_b*atomic_rad*2.0;  
    double fid_vol = Pi*w_a*w_b*1/(cross_section*n);
    double n3 = flux/v;
    double num_fid = n3*fid_vol;
